...
?>
    ...
    <img src="images/logo-blue.png" style="height:100px;" class="w3-right w3-hide-small w3-hide-medium w3-animate-top">
        <h1 class="header-title p-t-45"><b>Choose wordlist to add entry</b></h1>
        <br>
    </div>
  </header>
...
while ($wordlist = $resultWordlist->fetch_assoc()) {
  ...
  if ($entryCount > 0) {
    $percentageComplete = round(($recordedCount / $entryCount) * 100);
  }
  ...
  echo '<a href="add-entry.php?project_id=' . $projectID . '&wordlist_id=' . $wordlist['wordlist-id'] . '">';
  ...
  echo '<h4 class="w3-text-dark-grey"><i>' . $entryCount . ' entries in total';
  echo '</div>';
  echo '</div>';
  echo '</button>';
  echo '</a>';
