...
<div class="w3-main" style="margin-left:300px">
  ...
  <header id="projects">
    ...
    <?php if (isset($project)) { ?>
        <h1 class="header-title p-t-45"><b>Add entry to <i><?php echo $wordlist['wordlist-name'] ?></i></b></h1><br>
    <?php } ?>
  ...
  <div class="w3-container w3-padding-large w3-pale-grey">
    <form method="POST" id="add-entry">
      ...
      <div class="w3-section">
        ...
        <input class="w3-input w3-border" type="text" id="entry-pos" name="entry-pos" placeholder="Enter grammatical class of the word above" required>
      </div>
      <div class="w3-section">
        <label><h4>Category (Optional)</h4></label>
        <input class="w3-input w3-border" type="text" id="entry-cat" name="entry-cat" placeholder="This can be a part of speech specification or semantic category">
      </div>
      <div class="w3-section">
        <label><h4>Semantic Meaning</h4></label>
        <textarea class="w3-input w3-border" id="entry-def" name="entry-def" rows="5" cols="50" placeholder="Enter the definition or description of the word above"></textarea>      </div>
      <div class="text-center sign-message sign-message-error" id="pass-error">
					</div><br>
      <button type="submit" class="w3-button w3-black w3-margin-bottom"><h5>Add the Entry</h5></button>
    </form>
...
if (isset($_POST['entry-word']) && isset($_POST['entry-pos']) && isset($_POST['entry-cat']) && isset($_POST['entry-def'])) {
  ...
  $query = "INSERT `lexicon`.`entry` SET";
  $fields = array();
  ...
  if (!empty($fields)) {
      ...
      header("Location: add-to-list.php?project_id=".urlencode($projectID));
      ob_end_flush();
