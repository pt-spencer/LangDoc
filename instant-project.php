...
<html>
   ...
   <body class="w3-light-grey w3-content" style="max-width:1600px">
      ...
      <div class="w3-container">
      <form name="pronuncitation" action="instant-project.php?project_id=<?php echo $projectID ?>&wordlist_id=<?php echo $wordlistID ?>" method="POST" onsubmit="return validateForm()">
      ...
      <div>
        ...
        <h3><b>Informant </b></h3>
        <?php 
        $sqlIFM = "SELECT DISTINCT i.`informant-name`, i.`informant-id`
                FROM `lexicon`.`pronunciation` p
                JOIN `lexicon`.`entry` e ON p.`entry-id` = e.`entry-id`
                JOIN `lexicon`.`informant` i ON p.`informant-id` = i.`informant-id`
                WHERE e.`wordlist-id` = $wordlistID";
        $resultIFM = mysqli_query($mysqli, $sqlIFM);

        // Create a radio list of informants
        if ($resultIFM->num_rows > 0) {
            echo '<select name="informant" id="informant-select" style="height: 30px">';
            while($ifm = $resultIFM->fetch_assoc()) {
                $informant_id = $ifm["informant-id"];
                $informant_name = $ifm["informant-name"];
                echo "<option name='informant' id='informant' value='$informant_id'>$informant_name</option>";
            }
            echo '<option value="no-informant">---New Informant---</option>';
            echo '</select>';
        } else {
            echo '<select name="informant" id="informant-select">';
            echo '<option value="no-informant">---New Informant---</option>';
            echo '</select>';
        }
?>
           </div>
           <div class="w3-container w3-padding-large w3-pale-grey" id="informant-data-group" style="display: none;">
        ...
        <br>
        <label><h3><b>Word</b></h3></label>
        <input class="w3-input w3-border" type="text" id="entry-word" name="entry-word" placeholder="Enter head word or morpheme" required>
      </div>
         <div class="w3-col" style="width: 40%">               
         <label><h3><b>IPA</b></h3></label>
               ...
               <?php
                  ...
                  if (isset($_POST['IPA'])) {
                          $word = $_POST['entry-word'];
                          $wordlistID = $wordlistID;
                          $sqli = $mysqli->prepare("INSERT INTO `lexicon`.`entry` (`word`, `wordlist-id`) VALUES (?, ?)");
                            $sqli->bind_param("si", $word, $wordlistID);
                            $sqli->execute();
                          
                          $getword = "SELECT * FROM `lexicon`.`entry` WHERE `word` = '$word' AND `wordlist-id` = '$wordlistID'";
                            $result = mysqli_query($mysqli, $getword);
                            $entry = $result->fetch_assoc();
                            $entryID = $entry['entry-id'];

                          $IPA = $_POST['IPA'];
                          $informantID = $_POST['informant'];
                          $collectorID = $_SESSION['user-id'];
                          $note = $_POST['comment'];
                          
                          if($informantID == 'no-informant') {
                            $informant_name = $_POST['informant-name'];
                            $sex = $_POST['sex'];
                            $stmt = $mysqli->prepare("INSERT INTO `lexicon`.`informant` (`informant-name`, `informant-sex`) VALUES (?, ?)");
                            $stmt->bind_param("ss", $informant_name, $sex);
                            $stmt->execute();

                            $getinformant = "SELECT * FROM `lexicon`.`informant` WHERE `informant-name` = '$informant_name' AND `informant-sex` = '$sex'";
                            $result = mysqli_query($mysqli, $getinformant);
                            $informant = $result->fetch_assoc();
                            $informantID = $informant['informant-id'];
                          }
...
const informantSelect = document.getElementById('informant-select');
const informantDataGroup = document.getElementById('informant-data-group');
let previousValue = informantSelect.value;

if (informantSelect.options.length === 1) {
  informantDataGroup.style.display = 'block';
}

informantSelect.addEventListener('change', () => {
  if (informantSelect.value === 'no-informant') {
    informantDataGroup.style.display = 'none';
  } else {
    informantDataGroup.style.display = 'none';
  }
});
...
informantSelect.addEventListener('change', () => {
  if ($(informantDataGroup).is(':visible') && informantSelect.value !== 'no-informant') {
    $(informantDataGroup).slideUp();
  } else {
    $(informantDataGroup).hide();
  }

  previousValue = informantSelect.value;
});

$(document).ready(function() {
  // Add an event listener to the select element
  $('#informant-select').on('change', function() {
    // Check if the selected option has a value of "no-informant"
    if ($(this).val() === 'no-informant') {
      // Hide the "informant-data-group" div element
      $('#informant-data-group').slideDown();
    } else {
      // Show the "informant-data-group" div element
      $('#informant-data-group').hide();
    }
  });
});
