...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['remove-list'])) {
    ...
    $wordlistID = $_POST['remove-list'];
    // Delete all entries with the specified wordlist-id
    $sqlDeleteEntries = "DELETE FROM `entry` WHERE `wordlist-id` = $wordlistID";
    $resultDeleteEntries = $mysqli->query($sqlDeleteEntries);
    // Remove the wordlist from the database
    $sqlRemove = "DELETE FROM `wordlist` WHERE `wordlist-id` = $wordlistID";
    $resultRemove = $mysqli->query($sqlRemove);
    // Redirect to the same page to refresh the wordlist list
    header('Location: project-list.php?project_id=' . $projectID);
    exit;
  }
}
...
<h2><b>WORDLIST</b></h2>
  ...
  <?php
require_once 'db_connection2.php';
// Fetch all wordlists belonging to the current project
...
// Loop through each wordlist and display it
while ($wordlist = $resultWordlist->fetch_assoc()) {
  ...
  $sqlLemma = "SELECT COALESCE(COUNT(*), 0) AS lemma_count FROM `dictionary` WHERE `entry-id` IN (SELECT `entry-id` FROM `lexicon`.`entry` WHERE `wordlist-id` = {$wordlistID})";
  ...
  echo '<input type="hidden" name="remove-list" value="' . $wordlistID . '">';
  echo '<button type="submit" class="remove-list-btn"><i class="fa fa-minus-square fa-lg" style="color: #b00505;"></i></button>';
  echo '</form>';
  echo '</div>';

  echo '</div>';
}
if ($resultWordlist->num_rows > 0) {
  // code to generate buttons for each wordlist
} else {
  echo '<div class="w3-margin-bottom p-t-20 p-b-20"><h3 style="color: #b00505; text-align: center;"><i>No wordlists for the project</i></h3></div>';
}
?>
...
<div style="text-align: center;">
  <button onclick="window.location.href='project-home.php?project_id=<?php echo $projectID; ?>';" type="button" class="w3-button w3-oxford-blue w3-padding-40"><h4><b>← Back</b></h4></button>
</div>
</div>
</div>
